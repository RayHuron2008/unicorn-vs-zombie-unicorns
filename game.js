(() => {
  ...
  let state = {
    ...
    corruption: 0,
    enemies: [],
    bullets: [], // ray gun shots
    effects: [], // confetti/sparks
    specialTimer: 0, // ray gun duration in seconds
  };
  ...
  function setupJoystick() {
    ...
    window.addEventListener('resize', () => {
      const rect2 = joystick.getBoundingClientRect();
      joyCenter = { x: rect2.left + rect2.width/2, y: rect2.top + rect2.height/2 };
    });
  }
  ...
  function updateStick(cx, cy){
    ...
    const d = Math.min(maxR, dist(cx, cy, center.x, center.y));
  ...
  function spawnEnemy(special=false){
    ...
    const speed = special ? rand(1.1,1.6) : rand(0.8,1.4);
    state.enemies.push({
      x,y,vx:0,vy:0,size: 18, hp: special?3:1, special,
  ...
  function killEnemy(e){
    ...
    if(e.special){
      ...
      playBossMusic(); // fun: treat special like a hype moment
      // Auto-switch back to main after 30s
  ...
  function onVictory(){
    // Placeholder victory trigger (you can call this from a level clear event)
    if (!audio.enabled) return;
    try {
      audio.musicMain.pause();
      audio.musicBoss.pause();
