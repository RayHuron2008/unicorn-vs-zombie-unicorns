(() => {
  ...
  function addParticles(x, y, colorType = "rainbow") {
    for (let i = 0; i < 14; i++) {
      state.particles.push({
        ...
        vy: rand(-180, 60),
        life: rand(0.25, 0.65),
        hue: colorType === "red" ? rand(0, 25) : rand(0, 360)
      });
    }
  }
  ...
  function killEnemy(index) {
    ...
    if (state.mode === "final" && state.finalSpawned >= FINAL_RAY_COUNT && state.enemies.length === 0) {
      startNpcScene();
    }
  }
  ...
  function headbutt() {
    ...
    for (let i = state.enemies.length - 1; i >= 0; i--) {
      ...
      const box = { x: e.x - 27, y: e.y - 34, w: e.w, h: e.h };
  ...
  function updateShots(dt) {
    for (let i = state.playerShots.length - 1; i >= 0; i--) {
      ...
      for (let j = state.enemies.length - 1; j >= 0; j--) {
        ...
        if (dist(b.x, b.y, e.x, e.y - 25) < b.r + 24) {
    ...
    for (let i = state.enemyShots.length - 1; i >= 0; i--) {
      ...
      if (dist(b.x, b.y, player.x, player.y - 24) < b.r + 19) {
  ...
  function updateEnding(dt) {
    if (state.mode === "npc") {
      ...
      if ((state.npc.vx > 0 && state.npc.x >= target) || (state.npc.vx < 0 && state.npc.x <= target)) {
        state.npc.x = target;
  ...
  function updateHud() {
    ...
    if (timeEl) {
      const remaining = state.mode === "play" ? Math.max(0, Math.ceil(LEVEL_TIME - state.time)) : 0;
    ...
    if (powerFillEl) powerFillEl.style.width = `${Math.floor(clamp(pct, 0, 1) * 100)}%`;
    if (powerLabelEl) powerLabelEl.textContent = label;
  }
