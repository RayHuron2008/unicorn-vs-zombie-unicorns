(() => {
  ...
  if(resumeBtn){ resumeBtn.addEventListener('click', () => { setPaused(false); startAudioIfNeeded(); }); }
  ...
  function draw(now){
    ctx.save(); ctx.globalAlpha=0.9; ctx.fillStyle='#fff'; ctx.font='14px system-ui, sans-serif';
    ctx.fillText(__UVZU_VERSION__, 8, 18);
    ctx.fillText(`dx:${input.dx.toFixed(2)} dy:${input.dy.toFixed(2)} running:${state.running}`, 60, 18);
    ctx.restore();
  ...
  init();
  setTimeout(()=>{ try{ setPaused(false); }catch(e){} }, 0);
})();