// Level 6, revision 2: truck abductions and the mech's capture victory.
...
(() => {
  function catchersRuntime() {
    ...
    const fresh = () => ({ level: levelCode, session: session(), run: Date.now() + "-" + (++serial),
      ...
      hauls: {}, capture: null, truckArrival: 0 });
    hunt = fresh();
    ...
    window.__uvzuGetCatcherStatus = () => active() && trap ?
      { kind: trap.kind, time: trap.time, taps: trap.taps, run: hunt.run, source: trap.source,
        id: trap.id, x: trap.x, y: trap.y } : null;
    ...
    fullRestart = function() {
      ...
      seenHits.clear(); lastRequest = -99; requestSerial = trapSerial = 0; noticeTime = 0; appliedLoss = "";
      if (active()) {
    ...
    function contestants() {
      return allPlayers().filter(p => {
        ...
        return !(hunt.capture?.victims[p.role]) &&
          ...
          p.catcherTrap?.kind !== "captured" && p.catcherTrap?.kind !== "cage";
      });
    }
    ...
    function beginHaul(h, p) {
      ...
      // The lift must finish before paralysis wears off. A late arrival is escapable.
      if (!status || status.kind !== "stun" || status.run !== hunt.run || status.time < 0.7 ||
    ...
    function updateHaul(h, haul, dt) {
      ...
      haul.time += dt;
      if (haul.mode === "lift") {
        if (status.kind !== "stun" || status.time <= 0) { releaseHaul(haul); return; }
    ...
    function finishCaptureLoss() {
      ...
      if (!ghost()) {
        player.lives = 1; player.invuln = 0; state.mode = "play"; loseLife();
        state.resetQueued = false; player.actionLock = 0; oldLives = player.lives;
        const heading = document.getElementById("gameOverOverlay")?.querySelector?.("#pausePanel > div");
        if (heading) heading.textContent = "YOU WERE CAPTURED";
    ...
    function updateHumans(dt) {
      ...
      for (const h of hunt.humans) {
        ...
        if (Math.abs(dx) < 31 && Math.abs(dy) < 29 && beginHaul(h, p)) continue;
        if (h.mode === "windup") {
          if (h.timer <= 0) {
        ...
        for (const o of hunt.humans) {
          if (o === h || o.hp <= 0 || o.mode === "lift" || o.mode === "carry") continue;
      ...
      if (hunt.defeated === roster.length && !Object.values(hunt.hauls).some(a => a.mode === "lift" || a.mode === "carry")) phase("mechEntry");
    }
    ...
    function capturePlayer() {
      ...
      if (!ghost()) { safeLifeReset(); note("ROPE HELD — ONE LIFE LOST. KEEP MOVING!", 2.2); }
    }
    function applyTrap(kind, source, time) {
      ...
      trap = { kind, source, time: kind === "cage" ? 99 : time, taps: 0,
    ...
    function updateTrap(dt) {
      ...
      if (haul?.id === trap.id && (trap.kind === "stun" || trap.kind === "carry")) {
    ...
    updateHud = function() {
      ...
      timeEl.textContent = captureScene() ? "Captured" : hunt.phase === "humans" ? "Catchers " + hunt.defeated + "/8" :
    ...
    function drawHuman(h) {
      ...
      const moving = h.mode === "move" || h.mode === "carry", stride = moving ? Math.sin(hunt.clock * 7 + h.id) * 5 : 0;
    ...
    function addCaptureActors(actors) {
      ...
      for (const p of Object.values(scene.victims)) {
        ...
        const progress = scene.kind === "truck" ? clamp(t / 1.1, 0, 1) : 0;
        actors.push({ y: H, draw: () => {
          ctx.save();
          if (scene.kind === "truck") {
            ctx.translate(lerp(p.x, door.x + 55, progress), lerp(p.y, door.y - 50, progress));
    ...
    draw = function() {
      ...
      if (trap && !captureScene()) {
        ...
        } else text(trap.kind === "stun" ? "PARALYZED!  " + Math.max(0, trap.time).toFixed(1) + "s" :
          trap.kind === "carry" ? "BEING CARRIED TO THE TRUCK!" : "LASER CAGE — CAPTURED!", W / 2, y + 13, 22, "#bfeaff");
