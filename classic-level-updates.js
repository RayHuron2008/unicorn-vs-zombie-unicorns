// Permanent early-level improvements, version 2.
...
// Spider webs break shields, cannot extend a trap, and fire every five seconds.
// Load before game.js; install after the level additions and Rainbow final fix.
(() => {
  function updateSpiderWeb(dt) {
    const wasWebbed = player.webbedTimer > 0;
    player.webbedTimer = Math.max(0, (player.webbedTimer || 0) - dt);
    player.webFlash = Math.max(0, (player.webFlash || 0) - dt);
    player.webEscapeTimer = Math.max(0, (player.webEscapeTimer || 0) - dt);
    if (window.__uvzuCurrentLevelCode === "GRV2" && wasWebbed && player.webbedTimer <= 0) {
      player.webFlash = 0;
      player.webTrapX = player.webTrapY = null;
      player.actionLock = 0;
      // Give the player room to move even if a second spider has a web in flight.
      player.webEscapeTimer = 0.75;
    }

    window.__uvzuApplySpiderWebTrap = function(target) {
      if (!target || target !== player || window.__uvzuCurrentLevelCode !== "GRV2" ||
          target.lives <= 0 || window.__uvzuIsLocalGhost?.() ||
          (state.mode !== "play" && state.mode !== "final")) return false;
      if (target.invuln > 0 || target.dodgeTimer > 0) return false;
      if (shieldBlockContact()) {
        target.webbedTimer = target.webFlash = 0;
        target.webTrapX = target.webTrapY = null;
        target.actionLock = 0;
        target.webEscapeTimer = 0.75;
        updateHud();
        return false;
      }
      // More web hits cannot restart the same trap or cancel the escape window.
      if (target.webbedTimer > 0 || target.webEscapeTimer > 0) return false;
      target.webbedTimer = target.webFlash = 2.6;
      target.actionLock = 2.6;
      target.headTimer = target.dodgeTimer = 0;
      target.dodgeCooldown = Math.max(target.dodgeCooldown || 0, 0.5);
      target.webTrapX = target.x;
      target.webTrapY = target.y;
      return true;
    };
  }
  ...
  function classicRuntime() {
    ...
    fullRestart = function() {
      previousRestart();
      if (classic("GRV2")) {
        player.webbedTimer = player.webFlash = player.webEscapeTimer = 0;
        player.webTrapX = player.webTrapY = null;
  ...
  window.__uvzuInstallClassicLevels = function(code) {
    ...
    once(`    player.webbedTimer = Math.max(0, (player.webbedTimer || 0) - dt);
 player.webFlash = Math.max(0, (player.webFlash || 0) - dt);
...
};`, '    (' + updateSpiderWeb.toString() + ')(dt);');
    once('webTimer: isSecondGroup ? 0.9 : 999', 'webTimer: isSecondGroup ? 5 : 999');
    // Normal/Hard have two web spiders. Stagger their opening shots.
    once('webTimer: isSecondGroup ? 2.2 : 999', 'webTimer: isSecondGroup ? 7.5 : 999');
    once('if (typeof e.webTimer !== "number") e.webTimer = rand(1.1, 1.8);',
      'if (typeof e.webTimer !== "number") e.webTimer = 5;');
    once('e.webTimer = rand(1.7, 2.5);', 'e.webTimer = 5;');
